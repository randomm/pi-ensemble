...
- **Discover memory types first** (vipune types are project-defined, not fixed): `vipune list --json | jq -r '.[] | .memory_type' | sort -u` (If this command fails, skip `--memory-type` filtering and proceed with `--hybrid` searches only.)
