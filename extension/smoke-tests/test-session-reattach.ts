...
// 1. Flag OFF → re-dispatch, even with a transcript present.
{
  ...
  const r = resolveReattach("explore", [{ jobId: "j1", transcriptPath: TS }], 1_000_000, 3_600_000, 990_000, {
    fs: present,
  });
...
// 2. Flag ON + single-dispatch + transcript present → reattach with --session.
{
  ...
  if (r.mode === "reattach") {
    ...
    assert(r.grantMs === 3_600_000 - (1_000_000 - 990_000), "grant = original timeout minus elapsed");
    assert(
...
// 3. Fan-out step, flag on → re-dispatch (no --session).
{
  ...
  for (const step of ["develop", "lens-review", "adversarial"] as const) {
    const r = resolveReattach(step, [{ jobId: "j1", transcriptPath: TS }], 1_000_000, 3_600_000, 990_000, {
      fs: present,
    });
  ...
  const explore = resolveReattach("explore", [{ jobId: "j1", transcriptPath: TS }], 1_000_000, 3_600_000, 990_000, {
    fs: present,
  });
...
{
  process.env.PI_ENSEMBLE_SESSION_REATTACH = "1";
  const r = resolveReattach("explore", [{ jobId: "j1" }], 1_000_000, 3_600_000, 990_000, { fs: present });
...
// 6. Grant clamps to the 5-min floor.
{
  ...
  const r = resolveReattach("explore", [{ jobId: "j1", transcriptPath: TS }], 1_000_000, 600_000, 400_000, {
    fs: present,
  });
  assert(
  ...
  const r2 = resolveReattach("explore", [{ jobId: "j1", transcriptPath: TS }], 1_000_000, undefined, undefined, {
    fs: present,
  });
  assert(
