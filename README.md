...
| `/research <topic>` | Runs the compiled research spine (`start_research_driver`): memory inventory → parallel angle retrieval (structured claims, one `explore` child per angle) → **deterministic verification** (URL liveness; codebase claims grounded against the pinned commit) → a **dated artifact + provenance sidecar** under `outputs/` → a typed vipune row that supersedes the prior run on the same topic. Tiers: `quick` (1 angle) / `standard` (default) / `deep` (adds one scoped LLM entailment pass — per-claim source-support annotations, explicit and never a silent quality claim) / `adoption` (OSS decision memo: Scorecard-style signals + alternatives + this-repo integration fit, with a synthesized recommendation and comparison table embedded in the artifact). Zero verified findings writes an honest abstention artifact instead of a confident fabrication. PM keeps judgement: angle choice and the dig-deeper conversation after the artifact. |
| `/plan <description>` | Drafts a GitHub issue from your input — auto-classifies as bug/feature/epic/chore/spike, applies the right template, asks before creating. Runs as a compiled pipeline (`start_plan_driver`): a **deterministic pre-triage** rejects a too-thin descriptor with targeted questions in seconds (before any dispatch); typed context blocks (ACCEPTANCE CRITERIA / PITFALLS / OUT OF SCOPE / TEST SURFACE / DECOMPOSITION) are the operator's trusted channel into the spec, and an "EXACTLY N sub-issues" pin is threaded into the epic decomposition and asserted; the duplicate-risk check and the investigation angles dispatch as **one parallel barrier** (wall clock = slowest child, not their sum); every child is bounded at 30 minutes instead of the 2-hour spawn backstop; the drafted body passes **deterministic validation** (real acceptance criteria, real epic decomposition) before any LLM review; the adversarial gap gate runs for bug/feature/epic (chore/spike are low-blast-radius — validation is their gate), with round 2 a **scoped verification** of the carried CRITICAL resolutions rather than a second full review (an unparseable review fails closed — one strict retry, then not fileable — never a silent pass); the result reports per-phase timings so you can see where a slow ticket spent its time; and bodies are budgeted to the forge's 65,536-char limit — compacted deterministically (with a visible disclosure line, before the gate reviews), or halted with a per-section size breakdown — never rejected by the forge after the spend. |
...
| `/research <topic>` | Investigate a topic: web + codebase + memory | Any topic | Deterministic verification (liveness + commit pinning) | Dated artifact + provenance in `outputs/`; one typed candidate row (supersedes prior) |
