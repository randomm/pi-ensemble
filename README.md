...
| `/work <issue#> [<issue#> ...]` | Runs an issue end-to-end through a **compiled state-machine driver** (`extension/src/work-driver.ts`): explore → plan → feature branch (with optional parallel worktrees for multi-workstream decomposition) → developer dispatch → **per-workstream `adversarial_loop` gate** (parallel for N>1) → ops commits → PR → **six-pass code review** → CI watch → **auto-merge per project AGENTS.md merge policy**. Accepts multiple issue numbers (`/work 561 562`) to bundle related issues into a single PR; explore returns per-issue verdicts and the driver skips ALREADY_COMPLETE / NEEDS_CLARIFICATION issues, surfacing them in the PR body. Resumable state at `.pi/work-state/<primary>.json`; cap-hits surface as structured handoff comments with verbatim recovery commands. |
