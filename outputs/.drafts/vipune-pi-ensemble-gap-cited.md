...
- No source files contain commands such as `vipune add` or `vipune search`, nor is the optional MCP server subcommand referenced in `agents.json` or runtime code. (Search results show no matches beyond docs.)
