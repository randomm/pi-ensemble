...
- **To create any issue, call `start_plan_driver`** with the descriptor (and optionally `type` and prior `context`) — it files against whichever forge the repo is on. The `context` param is the TRUSTED channel: typed blocks (ACCEPTANCE CRITERIA / PITFALLS / OUT OF SCOPE / TEST SURFACE / DECOMPOSITION headings) override specialist output for those fields, an "EXACTLY N sub-issues" pin is enforced, and an acknowledgment like "this deliberately reverses #103" reconciles a duplicate-risk block. Direct `gh issue create` / `glab issue create` — bare, `oo`-wrapped, or chained — is structurally refused for every role in every mode; the refusal names the tool to use instead.
...
The driver produces tracker issues whose **body is the canonical spec** for downstream `/work` cycles. Tickets that drive working code need acceptance criteria, anti-rediscovery references, named pitfalls, and explicit Open Questions — not just a one-liner. The compiled phases: classify (regex), deterministic pre-triage (a too-thin descriptor — under the word floor, no code identifier, no context — returns targeted questions before any dispatch), inventory (vipune + `gh issue list`, mechanically), investigation (the duplicate-risk explore and the type-specialised angles dispatch as ONE parallel barrier: bug → reproduction + affected-code + test-surface; feature → prior-art + interfaces-and-contracts + test-surface; epic → decomposition + success-criteria; chore → scope-validation + affected-files; spike → scoping), draft synthesis (the driver assembles the body), deterministic validation (real acceptance criteria / real epic decomposition, or the draft halts before any review), and the adversarial gap gate for bug/feature/epic only (one `@adversarial-developer` pass; CRITICAL-only blocks and gets one corrective iteration whose round 2 is a scoped verification of the carried resolutions; residual HIGH/MEDIUM/LOW travel with the spec — a cap hit, not a loop; chore/spike skip the gate by design — deterministic validation is their gate).
...
**Adversarial gap gate is mandatory for bug/feature/epic.** Every such draft spec gets pressure-tested by `@adversarial-developer` before user confirmation. Only CRITICAL gaps block and trigger one corrective iteration (cap at 2 to prevent doom loops; round 2 is a scoped verification of the carried resolutions, never a second full review); HIGH/MEDIUM/LOW travel with the spec as residual disclosure or Open Questions. Chore/spike skip the gate — the driver's deterministic validation is their gate.
