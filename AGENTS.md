...
`agents-base/developer.md`, `agents-base/ops.md`, `agents-base/explore.md` carry a "Scratch hygiene" section each so even PM-driven (`PI_ENSEMBLE_WORK_DRIVER=0`) dispatches respect the convention. The inline prompts the driver sends thread the absolute scratch-dir path into every dispatch so subagents see the expected location verbatim.

**/work runs through the compiled driver by default** (Option C v1, this PR). `extension/src/work-driver.ts` owns the step-transition table walked verbatim from `pi-prompts/work.md` (explore → plan → branch → develop → adversarial → commit-pr → lens-review ± lens-fix ± step-back → ci → merged/handoff). The `merged` terminal step actually executes `gh pr merge` per project AGENTS.md merge policy (PR10 — was a 0ms no-op pre-fix; failure routes through `step-failed:merged` cap-hit → handoff). `/work` accepts multiple issue numbers (`/work 561 562 563`) and as of PR16 runs a **deterministic grouping analysis** first — `groupIssues()` in `work-driver.ts` partitions the N issues into K groups using explicit rules (R1 link markers `Depends-on: #N` / `Blocked-by: #N`, R2 path-overlap Jaccard ≥ 0.5 on extracted file paths, R3 SPLIT markers, R4 subsystem-tag prefixes `[frontend]`, R5 default separate). Related issues share ONE PR (via the PR10 bundled driver-level API `ctx.issues=[N,M]`); unrelated issues run as separate cycles. **#368 — park-and-continue between groups**: a group ending non-`merged` is recorded with its cap, failing step and the required human action, and the queue moves on; only a systemic failure halts everything (`rate-limited:quota-window` / `quota-terminal`, or a driver throw), because only those make the next group's attempt pointless. Pre-#368 any failure halted the queue — `/work` over 13 issues died on #279 and left 11 unrelated groups unstarted, and 69% of halt-triggering failures are provider infrastructure (#366) that says nothing about the remaining work. Escape hatch `PI_ENSEMBLE_QUEUE_HALT_ON_FAILURE=1`. Restores the old PM-driven `/work`'s "analyze first, decide the plan" shape but in pure code — no LLM narrating the grouping. Guardrail: components > `MAX_ISSUES_PER_GROUP` (3) split into singletons (empirical convergence ceiling from vipune `37219c9a`). Pre-PR16 (v0.12.5–v0.12.15): PR10 bundled ALL issues into one PR (failed 3× empirically); PR15 retreated to strictly sequential one-PR-per-issue (safe but ignored related-issue signal). PR16's grouping restores the smart middle path. PR11: lens-review now fetches `git diff origin/<base>..HEAD` so it actually runs post-commit (pre-PR11 fired the empty-diff guard in 34ms on every cycle, merging code without six-pass review); #286: adversarial now has the same empty-diff skip (PR6 had it for lens, adversarial didn't — full reviewer spawns on empty diffs were pure waste). Escape hatch `PI_ENSEMBLE_ADVERSARIAL_EMPTY_SKIP=0`. develop + speculative-explore prompts thread the NEEDS_WORK issue list (not the primary cycle issue); and explore halts with cap `explore-bodies-empty` on a pre-condition failure when `gh issue view` returns empty for any requested issue (e.g., projectCards GraphQL deprecation, gh extension hijack, auth lapse). PR12: `/work N --restart` (order-independent flag) wipes the prior state file and runs a fresh cycle — pre-PR12 re-invocation on a terminal state (handoff/aborted/merged) silently fell through with no notify and PM ended up recommending `/do` as a workaround. The step-back path now synthesises a `step-back-revise-spec` cap-hit after the @explore SDD analysis, surfacing the parsed `sddElement / diagnosis / proposedRevision` in the handoff with concrete `/plan N` + `/work N --restart` recovery commands. Terminal-state re-invocation without `--restart` now emits a clear notify pointing at the restart command. PR14: commit-pr's prompt now enumerates every worktree + workstream for N>1 cycles and walks ops through an explicit `git apply --index` consolidation recipe; a post-dispatch gate then verifies the committed diff includes files from EVERY workstream's `paths` and halts with cap `commit-pr-incomplete-consolidation` on partial commits. Pre-PR14 the prompt was single-tree shaped and ops silently dropped sibling worktrees' changes (live evidence: v0.12.13 /work 577 closed an issue with 1 of 3 workstreams shipped — root fix lost from main while the issue marked COMPLETED). PR13: the explore step now fetches issue bodies as a BARRIER before the dispatch and embeds the body content inline in the explore prompt — pre-PR13 the fetch ran in parallel with the dispatch (PR3 Pattern 1) and the prompt never inlined the body or pointed at the cached artifact, so the agent's verdict committed before the body was readable and produced false NEEDS_CLARIFICATION cap-hits (empirically /work 561 and /work 563 565 on v0.12.12 with substantive bodies present). PR11's empty-body halt also moves above the dispatch, so doomed cycles don't burn explore tokens. The driver dispatches subagents directly via `dispatchCore()` (in `dispatch.ts`) with `ownerKind: "driver"` so async-jobs skips the steer-back to PM; the driver awaits the completion promise and routes the result through the state machine. Workflow state persists to `<project>/.pi/work-state/<issue>.json` (schema v1, mandatory; discriminated union of pipelineState + append-only eventLog). The driver is the orchestrator for /work; PM is the chat-side reporter that surfaces sendUserMessage progress lines from the driver to the user. Other commands (/research, /audit, /plan, /review, /start, /do) stay PM-driven — they're prose-orchestrated by design (see plan file's command taxonomy table). `/do <description>` specifically is the free-form counterpart to `/work`: same orchestration toolkit (dispatch_specialist + adversarial_loop + dispatch_lens_review), no compiled driver, no GitHub issue requirement. Use it when you have lens-review findings or other context in scrollback that you want to act on without filing an issue first. PR15 also raises the `ci` step's ops timeout to 30 min (was 10 min via `ROLE_TIMEOUT_DEFAULTS_MS.ops`) — real CI runs regularly exceed 10 min and pre-PR15 they SIGTERM'd mid-`gh run watch` and routed through `step-failed:ci` handoff (3× this session). Override with `PI_ENSEMBLE_CI_WATCH_TIMEOUT_MS` (ms). PR17: **driver-side outcome-verification gates** (`verifyStepOutcome` in `work-driver.ts`) — after `develop` (all branches claiming success) and after `commit-pr` (consolidation gate passed), the driver checks EXECUTED evidence instead of trusting the subagent's "done" claim: develop = at least one worktree has a real diff (porcelain or commits ahead of `baseSha`, recorded at branch step) AND the project's verify command exits 0 (discovered via `.pi/verify-cmd` > package.json `typecheck`/`test` script with lockfile-detected runner > `Cargo.toml` → `cargo check --quiet`; `PI_ENSEMBLE_VERIFY_TIMEOUT_MS` caps it, default 10 min); commit-pr = commits exist ahead of `origin/<base>` AND the PR number resolves via `gh pr view` (missing `pr:` marker triggers a `gh pr list --head` repair that adopts the found number). Failure → cap `verify-failed:<step>` → handoff with per-check evidence in `pipelineState.verifyEvidence`. Zero LLM tokens; catches hollow claims pre-commit instead of post-PR CI (MAST: verification failures = 21.3% of multi-agent failures; the #245/#253 silent-merge incidents were this class). Escape hatch `PI_ENSEMBLE_VERIFY=0`. PR18 hardened the gates pre-live: R1 (one worktree's git-status error no longer suppresses the hollow-diff check for the others — per-worktree assessability tracking), R6 (Cargo.toml beats a bare package.json `test` script so Rust repos with tooling package.jsons don't run `npm run test` as their gate; an explicit `typecheck` script still wins), R4 (end-to-end integration tests of the cap wiring). PR277: added skip-ratchet (net additions of `#[ignore]`, `it.skip`, etc. fail the develop gate to prevent disabling test gates) and optional product smoke command (`.pi/smoke-cmd` at repo root, executed with the same timeout as the verify command). Both run in the develop verification path; escape hatches `PI_ENSEMBLE_SKIP_RATCHET=0` and `PI_ENSEMBLE_SMOKE=0`. PR19: **mechanized commit-pr** — `mechanizedCommitPr()` executes the consolidation recipe as driver code (per-worktree stage → `git diff --cached` → `git apply --index` at repoRoot → templated commit with `Fixes #N` lines → push → `gh pr create --body-file` → PR number parsed from the URL) instead of narrating it to an LLM ops dispatch. Untracked new files are now captured (staged before diff — `git diff HEAD` alone missed them). ANY mechanized failure emits a plumb-report and falls back to the unchanged PR14 LLM ops dispatch (judgmental recovery for env variance); both paths share the same downstream `parsePrNumber` + consolidation + verification gates. Escape hatch `PI_ENSEMBLE_MECHANIZE_OPS=0`. **#378 — intent resolution before planning**: the driver no longer assumes an issue says what to build. It resolves the intent from ANY body (spec, paragraph, or one-liner), grounds it against the code and the world, and returns `proceed` / `proceed-with-assumptions` / `park` with a machine-readable reason (`underspecified`, `contradicted-by-code`, `already-implemented`, `too-large`, `premise-unsound`). A park halts at `intent-park` before plan or branch, so it costs one explore dispatch rather than a cycle; **a missing or unreadable verdict parks** — silence is no longer permission to build. Assumptions are surfaced in the PR body. The resolver runs in the `explore` role, which is structurally denied write/edit tools. Escape hatch `PI_ENSEMBLE_INTENT=0`. **#380 — merge authority + executed evidence**: the `merged` step had no authority gate at all and decided to merge on `text.includes("ci-status: success")` in an ops child's reply. It now requires BOTH an explicit grant (a recognised sentence in the project's `AGENTS.md`, or `/work N --merge`) AND evidence gathered from `gh` (`mergeStateStatus`, `gh pr checks`, `skipped`/`neutral` required checks treated as NOT passing). It fails closed — an unreadable `gh` blocks the merge — and with no grant the cycle parks as `awaiting-human-merge` with the PR open and the work pushed. At the `ci` step the weaker rule applies: narration cannot promote a status, only executed evidence can demote one. The same PR fixed `mechanizedMerge` reading `ctx.verifyExecFn` with no `?? execp` fallback — a TEST-only seam, so every production merge silently took the LLM path and `restoreCheckout` never ran. Escape hatch `PI_ENSEMBLE_MERGE_AUTHORITY=0`. **#382 — resume**: the resume machinery was declared but inert — `resumable: false` was a literal in the TYPE, `inFlightJobIds` was validated and rendered but never written, and `dispatch-started` was never emitted anywhere in `src/`, so the validator cross-checking them could only pass vacuously. State was persisted only at step boundaries while a dispatch can run 30 minutes, so a crash left the file at the previous boundary still claiming `running` — indistinguishable from a live cycle, forever. Every step now write-aheads (`beginDispatch` in `work-driver-resume.ts`) before awaiting, including the four that bypass `runSingleDispatch` (`explore`, `plan`, `develop`, `handoff` — `develop` is the longest step and the biggest window). Re-entry on a `running` file resolves to refuse (live owner), resume (dead owner + in-flight dispatch), or continue (clean boundary); an in-flight id with no `dispatch-started` is corrupt state and still halts. Queue outcomes persist to `.pi/work-state/queue-summary.json`, and `/work-status` with no argument now renders the multi-cycle index — `discoverAllCycles`/`renderCycleIndex` had no caller in `src/` before this. Escape hatch `PI_ENSEMBLE_RESUME=0`. **#386 — a recovered failure no longer halts the queue**: `isSystemicFailure` read the LAST `dispatch-failed*` event unconditionally, so a cycle that hit a quota window at `explore`, retried successfully, ran for another twenty minutes and then parked for an unrelated semantic reason still reported `systemic: true` — halting every remaining group, the exact outcome #368 exists to prevent. It now scans back only as far as the last successful `dispatch-completed`: a failure followed by a success was recovered and does not count. **#384 — the six-pass review no longer approves on the absence of evidence**: `runLens` treats an empty diff as approved, and `fetchIntegratedDiff` swallowed every git error and returned `""` — so a stale `origin/<branch>` ref, a transient git failure or a `maxBuffer` overrun merged code unreviewed. `readIntegratedDiff` now returns a discriminated result, and empty is established POSITIVELY via `git rev-list --count` rather than inferred from absent output; an unreadable diff halts with cap `lens-diff-unreadable` carrying the git error verbatim. Two pre-existing fixtures that claimed to cover the genuine no-work cycle never created `origin/<branch>` and were exercising a git failure instead — both now create the ref. **#279 — two verification tiers plus a widening scan**: the `ci` step runs `.pi/verify-cmd-full` (first non-empty non-comment line, verbatim, no derivation fallback) in the group's worktree before the CI watch, emitting a separate `verify-full-status` event so "fast green, full unrun" is visible rather than invisible — a failure bumps `ciRetryCount` and skips the ops dispatch that round; an absent file is `skipped`, never silent. Independently, `runLens` scans the integrated diff for removed compiler-enforced invariants (`T` → `Option<T>`, added `| null` / `?:`, removed `readonly`/`const`/`assert`/`pub`/`mut`, narrowing to `any`) and routes findings into the lens context with an explicit mandate for the ARCHITECTURE lens — route-only, it never fails a cycle. Escape hatches `PI_ENSEMBLE_VERIFY_FULL=0`, `PI_ENSEMBLE_WIDENING_SCAN=0`. Rationale: every worst-class incident (#245/#253, v0.12.13 partial consolidation) was LLM ops improvising enumerable git/gh operations — deleting the failure source beats detecting its failures. Set `PI_ENSEMBLE_WORK_DRIVER=0` to fall back to the legacy PM-driven /work flow (in-flight cycles from older versions, debugging escape hatch). Cap-hits, plumb-reports, and step-back triggers fire as structured events from the driver, not as prose PM judgments — the legacy /work.md prose for those (#233/#234/#235) still governs /plan + /audit which remain PM-driven. See `extension/src/workflow-state.ts` for the schema, `extension/src/work-driver.ts` for the state machine, and `docs/troubleshooting.md` for state-file recovery. **#388 — the operator gets told**: nothing `/work` produced reached a human outside the Pi session (`grep -rniE 'osascript|terminal-notifier|notify-send|webhook|slack'` over `src/` returned nothing; the GitHub comment + label fired only on `runHandoff`, not on a queue halt, `awaiting-human-merge` or a crash). `PI_ENSEMBLE_NOTIFY_CMD` now runs on those four states, once per group, never on a clean merge, with the message on stdin (never interpolated into the command — issue titles and provider errors are untrusted). It fails open in every direction: missing binary, non-zero exit and a hanging hook are all swallowed after a 5s timeout, and the queue outcome is byte-identical with a completely broken hook. **#384 — the six-pass review no longer approves on the absence of evidence**: `runLens` treats an empty diff as approved, and `fetchIntegratedDiff` swallowed every git error and returned `""` — so a stale `origin/<branch>` ref, a transient git failure or a `maxBuffer` overrun merged code unreviewed. `readIntegratedDiff` now returns a discriminated result, and empty is established POSITIVELY via `git rev-list --count` rather than inferred from absent output; an unreadable diff halts with cap `lens-diff-unreadable` carrying the git error verbatim. Two pre-existing fixtures that claimed to cover the genuine no-work cycle never created `origin/<branch>` and were exercising a git failure instead — both now create the ref. **#279 — two verification tiers plus a widening scan**: the `ci` step runs `.pi/verify-cmd-full` (first non-empty non-comment line, verbatim, no derivation fallback) in the group's worktree before the CI watch, emitting a separate `verify-full-status` event so "fast green, full unrun" is visible rather than invisible — a failure bumps `ciRetryCount` and skips the ops dispatch that round; an absent file is `skipped`, never silent. Independently, `runLens` scans the integrated diff for removed compiler-enforced invariants (`T` → `Option<T>`, added `| null` / `?:`, removed `readonly`/`const`/`assert`/`pub`/`mut`, narrowing to `any`) and routes findings into the lens context with an explicit mandate for the ARCHITECTURE lens — route-only, it never fails a cycle. Escape hatches `PI_ENSEMBLE_VERIFY_FULL=0`, `PI_ENSEMBLE_WIDENING_SCAN=0`. Rationale: every worst-class incident (#245/#253, v0.12.13 partial consolidation) was LLM ops improvising enumerable git/gh operations — deleting the failure source beats detecting its failures. Set `PI_ENSEMBLE_WORK_DRIVER=0` to fall back to the legacy PM-driven /work flow (in-flight cycles from older versions, debugging escape hatch). Cap-hits, plumb-reports, and step-back triggers fire as structured events from the driver, not as prose PM judgments — the legacy /work.md prose for those (#233/#234/#235) still governs /plan + /audit which remain PM-driven. See `extension/src/workflow-state.ts` for the schema, `extension/src/work-driver.ts` for the state machine, and `docs/troubleshooting.md` for state-file recovery. **#390 — `/start` reads the driver state**: it built session context from git, issues, PRs and CI but never looked at `.pi/work-state/`, so parked cycles and never-started groups were invisible at exactly the moment the operator decides what to do. `pi-prompts/start.md` now reads the state dir and `queue-summary.json` and leads the readiness line with parks and their actions. `test-start-prompt.ts` also enforces the file's bash rule (no `&&`/`|`/`;`/`>`/backticks/`$(…)`): `permission-guard` DENIES those silently rather than erroring, so a chained command disables its step with no visible symptom — the test is the only thing that catches it, and it carries an anti-vacuity check that the matcher really rejects what it claims to. **#386 — a recovered failure no longer halts the queue**: `isSystemicFailure` read the LAST `dispatch-failed*` event unconditionally, so a cycle that hit a quota window at `explore`, retried successfully, ran for another twenty minutes and then parked for an unrelated semantic reason still reported `systemic: true` — halting every remaining group, the exact outcome #368 exists to prevent. It now scans back only as far as the last successful `dispatch-completed`: a failure followed by a success was recovered and does not count. **#384 — the six-pass review no longer approves on the absence of evidence**: `runLens` treats an empty diff as approved, and `fetchIntegratedDiff` swallowed every git error and returned `""` — so a stale `origin/<branch>` ref, a transient git failure or a `maxBuffer` overrun merged code unreviewed. `readIntegratedDiff` now returns a discriminated result, and empty is established POSITIVELY via `git rev-list --count` rather than inferred from absent output; an unreadable diff halts with cap `lens-diff-unreadable` carrying the git error verbatim. Two pre-existing fixtures that claimed to cover the genuine no-work cycle never created `origin/<branch>` and were exercising a git failure instead — both now create the ref. **#279 — two verification tiers plus a widening scan**: the `ci` step runs `.pi/verify-cmd-full` (first non-empty non-comment line, verbatim, no derivation fallback) in the group's worktree before the CI watch, emitting a separate `verify-full-status` event so "fast green, full unrun" is visible rather than invisible — a failure bumps `ciRetryCount` and skips the ops dispatch that round; an absent file is `skipped`, never silent. Independently, `runLens` scans the integrated diff for removed compiler-enforced invariants (`T` → `Option<T>`, added `| null` / `?:`, removed `readonly`/`const`/`assert`/`pub`/`mut`, narrowing to `any`) and routes findings into the lens context with an explicit mandate for the ARCHITECTURE lens — route-only, it never fails a cycle. Escape hatches `PI_ENSEMBLE_VERIFY_FULL=0`, `PI_ENSEMBLE_WIDENING_SCAN=0`. Rationale: every worst-class incident (#245/#253, v0.12.13 partial consolidation) was LLM ops improvising enumerable git/gh operations — deleting the failure source beats detecting its failures. Set `PI_ENSEMBLE_WORK_DRIVER=0` to fall back to the legacy PM-driven /work flow (in-flight cycles from older versions, debugging escape hatch). Cap-hits, plumb-reports, and step-back triggers fire as structured events from the driver, not as prose PM judgments — the legacy /work.md prose for those (#233/#234/#235) still governs /plan + /audit which remain PM-driven. See `extension/src/workflow-state.ts` for the schema, `extension/src/work-driver.ts` for the state machine, and `docs/troubleshooting.md` for state-file recovery.
